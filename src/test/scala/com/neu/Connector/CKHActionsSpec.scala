...
class CKHActionsASpec extends AsyncFlatSpec with Matchers {
  ...
  it should "handle empty DataFrame write" in {
    ...
    actions.writeDataCKH(emptyDF, tableName)
      .map(_ => succeed)
  }

  it should "handle large batch writes" in {
    val largeData = spark.createDataFrame(
      spark.sparkContext.parallelize((1 to 2000).map(i => (s"key$i", s"value$i")))
    ).toDF("column1", "column2")


    actions.writeDataCKH(largeData, tableName)
      .map(_ => succeed)
  }

  it should "execute select query" in {
    actions.readDataCKH("SELECT 1 + 2", schema)
      .map(df => {
